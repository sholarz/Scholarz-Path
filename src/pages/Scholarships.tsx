          ...
          <Sheet>
            <SheetTrigger
              render={
                <Button
                  variant="outline"
                  className={`h-12 px-5 rounded-2xl border-slate-200 bg-white shadow-sm shrink-0 font-bold flex items-center gap-2 hover:border-indigo-200 hover:bg-white transition-all relative ${selectedField !== "all" || selectedCountry !== "all" ? "border-indigo-400 ring-2 ring-indigo-500/10 bg-indigo-50/10" : ""}`}
                >
          ...
          {filtered.map((s) => (
            ...
            >
              <div className="relative aspect-[16/10] w-full bg-slate-50 overflow-hidden">
                ...
                {s.imageUrl && <img src={s.imageUrl} alt={s.title} className="absolute inset-0 h-full w-full object-cover" referrerPolicy="no-referrer" />}
