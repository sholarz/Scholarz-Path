    ...
    <div className="mt-4 pt-4 border-t border-slate-50 space-y-4">
      <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 scrollbar-hide">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-400">
                    {comment.createdAt ? formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true, locale: id }) : 'baru saja'}
                  </span>
        ...
        {!loading && comments.length === 0 && (
          <p className="text-[10px] text-center text-slate-400 py-2 italic font-medium">Belum ada komentar. Jadi yang pertama membalas!</p>
        )}
      </div>

      <form onSubmit={handleSubmit} className="flex gap-2">
        <Input 
          className="h-8 text-xs rounded-full bg-slate-50 border-transparent focus:bg-white" 
            ...
            <form onSubmit={handlePost} className="space-y-6 pt-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea 
                  ...
                  className="min-h-[150px] rounded-2xl bg-slate-50 border-slate-100 p-4 focus:ring-slate-900/10"
            ...
            <Card className="border-red-100 bg-red-50/20 rounded-[40px] border-2 border-dashed py-12">
              <CardContent className="flex flex-col items-center justify-center text-center space-y-4">
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="mt-4 border-red-200 text-red-700 hover:bg-red-100 rounded-xl px-8 font-bold flex items-center gap-2"
                >
                ...
                <Card key={post.id} className="border-slate-100 shadow-sm hover:shadow-md transition-all rounded-3xl overflow-hidden group">
                  <CardContent className="p-6">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {post.role === 'admin' && (
                              <Badge className="bg-sky-50 text-sky-600 border-none px-2 py-0 h-4 text-[9px] font-black uppercase tracking-tighter">Admin</Badge>
                            )}
                            {post.category && (
                              <span className="text-[10px] font-black uppercase text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100/50">
                                {post.category}
                              </span>
                            )}
                          </div>
                        ...
                        <div className="flex items-center gap-1">
                          <Dialog open={postToDelete === post.id} onOpenChange={(open) => setPostToDelete(open ? post.id! : null)}>
                            ...
                            <DialogContent className="rounded-3xl max-w-[400px]">
                              ...
                              <div className="py-2 text-slate-500 font-medium">
                                Tindakan ini tidak dapat dibatalkan. Postingan Anda akan dihapus secara permanen.
                              </div>
                              <DialogFooter className="gap-2 sm:gap-0">
                                <Button variant="ghost" onClick={() => setPostToDelete(null)} className="rounded-xl font-bold">Batal</Button>
                                <Button 
                                  onClick={() => handleDelete(post.id!)} 
                                  disabled={sending}
                                  className="bg-red-500 text-white hover:bg-red-600 rounded-xl font-bold px-6"
                                >
                    ...
                    {expandedPosts[post.id!] && (
                      <CommentSection postId={post.id!} currentUser={user} isAdmin={isAdmin} />
                    )}
                  </CardContent>
                </Card>
              ))}
