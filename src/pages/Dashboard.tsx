...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // Calculate upcoming deadlines using shared filtering function
        // BUG-004 FIX: Frontend filtering ensures consistency between dashboard and roadmap
        // We fetch a larger set and filter on frontend because:
        // 1. Firestore date range queries require specific indexes
        // 2. Timezone-safe comparison is better handled on client side
        // 3. Active status filtering needs frontend logic anyway
        const scholarshipsQ = query(collection(db, "scholarships"), limit(100));
        const scholarshipsSnapshot = await getDocs(scholarshipsQ);
        const allScholarships = scholarshipsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        const upcomingScholarships = getActiveUpcomingDeadlines(
          allScholarships,
          30,
        );
        ...
        setStats({
          ...
          deadlines: upcomingScholarships.length,
        });
        ...
        setRecentPosts(
          forumSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
        );
      } catch (error) {
  ...
  const calculateCompletion = () => {
    ...
    const completed = fields.filter(
      (f) => !!profile[f] && profile[f] !== "0.0" && profile[f] !== 0,
    ).length;
  ...
  return (
    <div className="sp-page-container space-y-8">
      ...
      <div className="sp-page-header">
        ...
        <p className="sp-page-subtitle text-sm">
      ...
      {!isPremium && (
        <div className="bg-white rounded-2xl p-6 border border-amber-200 shadow-sm relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-amber-800/70 leading-relaxed mb-3">
                Get unlimited bookmarks, automated preparation timeline, and
                more!
              </p>
          ...
          <Button
            asChild
            className="bg-amber-500 text-white hover:bg-amber-600 font-bold px-8 rounded-xl shadow-md shadow-amber-100 self-start md:self-center"
          >
            <Link to="/premium">
              ...
              {loadingHistory ? (
                ...
                <div className="divide-y divide-slate-50">
                  {testHistory.map((result) => (
                    <Link
                      key={result.id}
                      to={`/test-prep?test=${result.testId || ""}`}
                    ...
                    >
                      ...
                      <div className="text-right">
                        <Badge
                          variant="outline"
                          className="rounded-lg bg-emerald-50 text-emerald-700 border-emerald-100 font-bold text-xs"
                        >
                          {result.percentage}%
                        </Badge>
                ...
                <div className="h-40 flex flex-col items-center justify-center text-slate-400 space-y-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    asChild
                    className="rounded-lg text-[10px] h-8 px-4 mt-2"
                  >
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm transition-all hover:shadow-md">
            ...
            <div className="space-y-3">
              <div
                className={`flex items-center justify-between p-3 border border-slate-100 rounded-xl transition-all ${profile?.institution ? "bg-emerald-50/50 border-emerald-100" : "bg-slate-50/50"}`}
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`p-1.5 rounded-lg ${profile?.institution ? "bg-emerald-100" : "bg-slate-200"}`}
                  >
                    <GraduationCap
                      className={`w-3 h-3 ${profile?.institution ? "text-emerald-700" : "text-slate-500"}`}
                    />
                  </div>
                  <span className="text-xs font-semibold text-slate-700">
                    {profile?.institution || "Institusi belum diisi"}
                  </span>
                </div>
                {profile?.institution && (
              ...
              <div
                className={`flex items-center justify-between p-3 border border-slate-100 rounded-xl transition-all ${profile?.englishScore ? "bg-emerald-50/50 border-emerald-100" : "bg-slate-50/50"}`}
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`p-1.5 rounded-lg ${profile?.englishScore ? "bg-emerald-100" : "bg-slate-200"}`}
                  >
                    <Globe
                      className={`w-3 h-3 ${profile?.englishScore ? "text-emerald-700" : "text-slate-500"}`}
                    />
                  </div>
                  <span className="text-xs font-semibold text-slate-700">
                    {profile?.englishScore || "Skor Bahasa (IELTS/TOEFL)"}
                  </span>
                </div>
            ...
            <Button
              asChild
              variant="outline"
              className="w-full py-2.5 mt-6 text-xs font-bold border-slate-200 text-slate-800 bg-white rounded-lg hover:bg-slate-50 transition-all flex items-center justify-center gap-2"
            >
              <Link to="/profile">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
            ...
            <div className="space-y-4">
              {recentPosts.length > 0 ? (
                recentPosts.map((post) => (
                  <Link
                    key={post.id}
                    to="/forum"
                    className="flex gap-3 pb-3 border-b border-slate-50 cursor-pointer hover:bg-slate-50 transition-colors rounded-lg p-1 group"
                  >
                    ...
                    <div>
                      <p className="text-[11px] font-bold group-hover:text-slate-900 line-clamp-1">
                        {post.content}
                      </p>
              ...
              <Button
                asChild
                variant="ghost"
                className="w-full text-[10px] text-slate-500 hover:text-slate-900"
              >
