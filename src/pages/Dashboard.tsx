      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { 
            ...
            color: "text-blue-500", 
            bg: "bg-blue-50",
            arrow: "text-blue-500 bg-blue-50"
          },
          { 
            label: "Bookmarked", 
            value: isPremium ? `${stats.bookmarks} / Unlimited` : `${stats.bookmarks}/3`, 
            icon: Bookmark, 
          ...
          <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              {stat.arrow && (
                <div className={`w-8 h-6 ${stat.arrow} rounded flex items-center justify-center`}>
                   ...
                   {testHistory.map((result) => (
                     <div key={result.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                       <div className="flex items-center gap-4">
                         ...
                         <div>
                           ...
                           <p className="text-[10px] text-slate-400 font-medium">
                             {result.createdAt?.toDate ? result.createdAt.toDate().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Baru saja'}
                           </p>
                         </div>
                       </div>
                       <div className="text-right">
                         ...
                         <p className="text-[10px] text-slate-400 mt-1 font-medium">{result.score}/{result.totalQuestions} Benar</p>
                       </div>
                     </div>
                   ))}
