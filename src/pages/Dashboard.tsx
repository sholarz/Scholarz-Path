...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      {/* Welcome Header */}
      <div className="pt-4">
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight italic">Welcome back, {user?.displayName || "User"}!</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          {
            label: "Bookmarked",
            value: `${stats.bookmarks}${isPremium ? "" : "/3"}`,
            subLabel: isPremium ? "Premium • tanpa batas" : null,
            icon: Bookmark,
          ...
          <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
            <div>
              ...
              {stat.subLabel && <p className="mt-0.5 text-[10px] font-medium text-slate-400">{stat.subLabel}</p>}
            </div>
                  ...
                  {testHistory.map((result) => (
                    <div key={result.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <Link to={`/test-prep?test=${result.testId || ""}`} className="text-sm font-bold text-slate-800 hover:text-indigo-600 transition-colors">
                            {result.testTitle}
                          </Link>
