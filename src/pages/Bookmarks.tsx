...
export default function Bookmarks() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight italic flex items-center gap-3">
          <Bookmark className="text-indigo-600" />
          Beasiswa Disimpan
        </h1>
        {isPremium && <p className="text-xs font-medium text-slate-400 mt-2">Premium • tanpa batas</p>}
      ...
      {loading ? (
            ...
            <Card key={b.id} className="overflow-hidden border-slate-100 shadow-sm hover:shadow-md transition-all group">
              <CardHeader className="pb-3 flex flex-row items-start justify-between gap-4 cursor-pointer" onClick={() => fetchScholarshipDetail(b.scholarshipId || b.id)}>
                <div className="space-y-1">
                  <CardTitle className="text-lg leading-tight font-bold text-slate-800 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{b.title}</CardTitle>
                ...
                >
                  {fetchingDetail && selectedScholarship?.id === (b.scholarshipId || b.id) ? (
                    ...
                    <span className="flex items-center gap-2">
                      Lihat Detail Beasiswa <ArrowRight size={14} />
                    </span>
                  )}
                </Button>
              </CardFooter>
        ...
        <div className="text-center py-24 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <p className="text-slate-500 mt-2 max-w-xs mx-auto text-sm">Jelajahi berbagai beasiswa menarik dan simpan yang paling sesuai dengan kriteria Anda.</p>
          ...
          {selectedScholarship && (
            <>
              <div className="p-8 bg-indigo-600 text-white">
                ...
                <DialogTitle className="text-3xl font-black leading-tight mb-2 pr-8">{selectedScholarship.title}</DialogTitle>
              ...
              <ScrollArea className="flex-grow">
                <div className="p-8 space-y-10">
                  <section>
                    <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 italic">Tentang Beasiswa</h3>
                    <p className="text-slate-600 leading-relaxed font-bold">{selectedScholarship.description}</p>
                  </section>
                  ...
                  {selectedScholarship.benefits && (
                    <section>
                      <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 italic">Benefit & Besaran</h3>
