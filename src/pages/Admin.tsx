  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 italic">Admin Control Tower</h1>
            ...
            <CardContent className="p-0">
              <TabsList className="flex flex-col w-full h-auto bg-transparent p-2 gap-1">
                ...
                <TabsTrigger value="payments" className="w-full justify-start px-4 py-3 data-[state=active]:bg-slate-100 data-[state=active]:shadow-none">
                  ...
                  {payments.filter((p) => p.status === "pending").length > 0 && <Badge className="ml-auto bg-amber-500">{payments.filter((p) => p.status === "pending").length}</Badge>}
                </TabsTrigger>
              </TabsList>
            </CardContent>
          </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Tabs value={formTab} onValueChange={setFormTab}>
                      ...
                      <TabsContent value="search" className="space-y-4">
                        <div className="flex gap-2">
                          <Input placeholder="Cari beasiswa di web..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleWebSearch()} className="h-9 text-xs" />
                              ...
                              <Card key={i} className="border-slate-100 shadow-none hover:border-indigo-200 transition-colors bg-white">
                                <CardHeader className="p-3 pb-1">
                                  <CardTitle className="text-[11px] font-bold flex justify-between items-start gap-2">
                                    ...
                                    {res.deadline && (
                                      <Badge variant="outline" className="text-[8px] h-4 shrink-0 font-normal">
                                        {res.deadline}
                                      </Badge>
                                    )}
                                  </CardTitle>
                                ...
                                <CardContent className="p-3 pt-0">
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <Button size="sm" variant="ghost" className="h-6 text-[9px] px-2 text-slate-400 hover:text-slate-600" asChild>
                                      <a href={res.sourceUrl} target="_blank" rel="noopener noreferrer">
                                        Buka Link
                                      </a>
                                    </Button>
                                  </div>
                        ...
                        <form onSubmit={handleAddManual} className="space-y-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <Textarea
                              ...
                              placeholder="Ringkasan tentang beasiswa ini..."
                            />
                          </div>

                          <div className="space-y-1">
                            <Label htmlFor="eligibility" className="text-xs">
                              Kriteria Kelayakan (Mendukung Markdown)
                            </Label>
                            <Textarea
                              id="eligibility"
                              className="text-xs min-h-[150px]"
                              value={manualData.eligibility}
                              onChange={(e) => setManualData({ ...manualData, eligibility: e.target.value })}
                              placeholder="Syarat IPK, usia, dll..."
                            />
                          </div>

                          <div className="space-y-1">
                            <Label htmlFor="benefits" className="text-xs">
                              Besaran & Benefit
                            </Label>
                            <Textarea
                              id="benefits"
                              className="text-xs min-h-[100px]"
                              value={manualData.benefits}
                              onChange={(e) => setManualData({ ...manualData, benefits: e.target.value })}
                              placeholder="Cth: Biaya Semester, Uang Saku..."
                            />
                          </div>

                          <div className="space-y-1">
                            <Label htmlFor="selectionProcess" className="text-xs">
                              Proses Seleksi
                            </Label>
                            <Textarea
                              id="selectionProcess"
                              className="text-xs min-h-[100px]"
                              value={manualData.selectionProcess}
                              onChange={(e) => setManualData({ ...manualData, selectionProcess: e.target.value })}
                              placeholder="Langkah-langkah pendaftaran..."
                            />
                          </div>

                          <div className="space-y-1">
                            <Label htmlFor="link" className="text-xs">
                              Link Pendaftaran (Asli)
                            </Label>
                      ...
                      <TabsContent value="ai" className="space-y-4">
                        <textarea className="min-h-[300px] w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-xs" placeholder="Tempel teks beasiswa..." value={rawText} onChange={(e) => setRawText(e.target.value)} />
              ...
              <div className="md:col-span-2">
                <Card>
                  ...
                  <CardContent>
                    {loading ? (
                      <Loader2 className="h-8 w-8 animate-spin mx-auto my-8 opacity-20" />
                    ) : (
                      <div className="space-y-3">
                              ...
                              <div className="flex items-center gap-2">
                                {confirmDeleteId === s.id ? (
                                  <div className="flex items-center gap-1 animate-in slide-in-from-right-2 duration-200">
                                    <Button size="sm" variant="ghost" onClick={() => setConfirmDeleteId(null)} className="text-[10px] h-7 font-bold">
                                      Batal
                                    </Button>
                                    <Button size="sm" onClick={() => handleDelete(s.id!)} disabled={deletingId === s.id} className="bg-red-500 hover:bg-red-600 text-white text-[10px] h-7 font-bold rounded-lg px-3">
                                      {deletingId === s.id ? <Loader2 className="h-3 w-3 animate-spin" /> : "Hapus"}
                                    </Button>
                                  </div>
                                ) : (
                                  <Button variant="ghost" size="icon" onClick={() => setConfirmDeleteId(s.id!)} className="text-slate-300 hover:text-red-500 h-8 w-8 rounded-lg hover:bg-red-50">
                                    <Trash2 size={14} />
                                  </Button>
                                )}
                              </div>
              ...
              <CardContent>
                {loadingPayments ? (
                  ...
                  <div className="space-y-4">
                    {payments.length === 0 ? (
                      <p className="text-center py-12 text-slate-400 text-sm italic">Belum ada permintaan pembayaran.</p>
                    ) : (
                      payments.map((p) => (
                        <div key={p.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 rounded-2xl border border-slate-100 bg-white gap-4">
                          <div className="flex gap-4">
                            <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-bold uppercase text-xs">{p.userName?.[0] || "U"}</div>
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <Badge className={p.status === "pending" ? "bg-amber-100 text-amber-700" : p.status === "approved" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}>{p.status.toUpperCase()}</Badge>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {p.status === "pending" && (
                              <>
                                <Button onClick={() => handleRejectPayment(p.id)} variant="ghost" size="sm" className="h-8 text-[10px] font-bold text-red-500 hover:text-red-600 hover:bg-red-50">
                                  Tolak
                                </Button>
                                <Button onClick={() => handleApprovePayment(p)} size="sm" className="h-8 text-[10px] font-bold bg-emerald-600 hover:bg-emerald-700 text-white">
                                  Approve & Upgrade
                                </Button>
                              </>
                            )}
